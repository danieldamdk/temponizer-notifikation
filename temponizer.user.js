...
// @version      7.2
// @description  Push (beskeder+interesse) m. leader; caller-pop kun INBOUND; Excel→CSV→GitHub; hover “Intet Svar”; drag panel; reset-knap; menu scroll; leader-hint live. XLSX-upload fjernet.
...
if (window.__TP72_RUNNING__) { console.info('[TP] already running'); } else { window.__TP72_RUNNING__ = true;

/* ─────────────── utils/log ─────────────── */
const TP_VER='7.2'; const _log=[]; const LOG_MAX=80;
function tpLog(l,...a){const ts=new Date().toLocaleTimeString();_log.push(`[${ts}] ${l}: ${a.map(x=>typeof x==='string'?x:JSON.stringify(x)).join(' ')}`);if(_log.length>LOG_MAX)_log.shift();try{console[l](...a);}catch(_){console.log(...a);}}
...
function isQuietNow(){const{on,from,to}=getQuietCfg();if(!on)return false;const m=s=>{const[a,b]=s.split(':').map(n=>+n||0);return a*60+b;};const now=new Date();const n=now.getHours()*60+now.getMinutes(), f=m(from), t=m(to);return f<=t?(n>=f&&n<t):(n>=f||n<t);}
...
/* http helpers + retry */
function withRetry(fn,{tries=3,delays=[2000,5000,10000]}={}){return(...args)=>new Promise((res,rej)=>{let n=0;const run=()=>fn(...args).then(res).catch(e=>{n++;if(n>=tries)return rej(e);setTimeout(run,delays[Math.min(n-1,delays.length-1)]);});run();});}
...
bc?.addEventListener('message',ev=>{if(ev.data?.type==='leader:update'){}});
...
async function fetchExcelAsCSVText(){await ensureXLSX();const tries=[{fn:tryExcelGET,params:'id=true&name=true&phone=true&cellphone=true&gdage_dato=i+dag'},{fn:tryExcelGET,params:'id=true&name=true&phone=true&cellphone=true'},{fn:tryExcelPOST,params:'id=true&name=true&phone=true&cellphone=true&gdage_dato=i+dag'},{fn:tryExcelPOST,params:'id=true&name=true&phone=true&cellphone=true'}];for(const t of tries){try{const ab=await t.fn(t.params);if(!ab||ab.byteLength<128)continue;const wb=XLSX.read(ab,{type:'array'});if(!wb.SheetNames?.length)continue;const csv=pickBestSheetCSV(wb);if(csv)return csv;}catch(e){tpLog('warn','[PB] Excel hentning',e);}}return null;}
async function fetchExcelAsCSVAndUpload(){const text=await fetchExcelAsCSVText();if(!text){showToastOnce('csv','Ingen rækker fra Temponizer – beholdt eksisterende CSV.');return;}const lines=text.trim().split(/\r?\n/).filter(Boolean);if(lines.length<2){showToastOnce('csv','Kun header – beholdt eksisterende CSV.');return;}const base64=b64Utf8(text);const {sha}=await ghGetSha(PB_OWNER,PB_REPO,PB_CSV,PBRANCH=PB_BRANCH);await ghPutFile(PB_OWNER,PB_REPO,PB_CSV,base64,'sync: Excel→CSV via TM (auto)',sha,PB_BRANCH);showToastOnce('csvok','CSV uploadet (Excel→CSV).');}

/* caller-pop (only inbound) */
let _lastPopAt=0;
async function callerPopIfNeeded(){try{const q=new URLSearchParams(location.search);const raw=q.get('tp_caller');if(!raw)return;const dir=(q.get('tp_dir')||'').toLowerCase();const inbound=(dir==='in')||/\*1500$/.test(String(raw));if(!inbound)return;if(Date.now()-_lastPopAt<3000)return;_lastPopAt=Date.now();const p8=normPhone(String(raw).replace(/\*1500$/,''));if(!p8){showToast('Ukendt nummer: '+raw);return;}const csv=await gmGET(RAW_PHONEBOOK);const {map}=parsePhonebookCSV(csv);const rec=map.get(p8);if(!rec)return;const url=`/index.php?page=showvikaroplysninger&vikar_id=${encodeURIComponent(rec.id)}#stamoplysninger`;showToast(`Åbner vikar: ${rec.name||'ukendt navn'} (${rec.id})`);location.assign(url);}catch(e){tpLog('warn','[CALLER]',e);}}

/* drag helpers with safe restore */
function draggable(el,key,handle){(handle||el).style.cursor='move';let sx=0,sy=0,ox=0,oy=0,moving=false;restorePos();function restorePos(){const saved=JSON.parse(localStorage.getItem(key)||'null');if(!saved)return;el.style.position='fixed';el.style.right='';el.style.bottom='';el.style.left=saved.left;el.style.top=saved.top;setTimeout(()=>ensureOnScreen(el,key,true),0);}function down(e){if(e.button!==0)return;const t=e.target;if(/(input|textarea|select|button|a)/i.test(t.tagName))return;moving=true;sx=e.clientX;sy=e.clientY;const r=el.getBoundingClientRect();ox=r.left;oy=r.top;el.style.width=r.width+'px';el.style.height=r.height+'px';el.style.position='fixed';el.style.right='';el.style.bottom='';document.addEventListener('mousemove',move);document.addEventListener('mouseup',up);e.preventDefault();}
function move(e){if(!moving)return;const nx=ox+(e.clientX-sx);const ny=oy+(e.clientY-sy);el.style.left=Math.max(6,Math.min(window.innerWidth-el.offsetWidth-6,nx))+'px';el.style.top=Math.max(6,Math.min(window.innerHeight-el.offsetHeight-6,ny))+'px';}
function up(){moving=false;localStorage.setItem(key,JSON.stringify({left:el.style.left,top:el.style.top}));document.removeEventListener('mousemove',move);document.removeEventListener('mouseup',up);}
(handle||el).addEventListener('mousedown',down);}
function ensureOnScreen(el,key,save){const r=el.getBoundingClientRect();let left=parseInt(el.style.left||'0',10);let top=parseInt(el.style.top||'0',10);let changed=false;
if(r.right<40||r.bottom<40||r.left>window.innerWidth-40||r.top>window.innerHeight-40){left=Math.max(6,Math.min(window.innerWidth-el.offsetWidth-6,left||window.innerWidth-el.offsetWidth-12));top=Math.max(6,Math.min(window.innerHeight-el.offsetHeight-6,top||window.innerHeight-el.offsetHeight-12));el.style.left=left+'px';el.style.top=top+'px';el.style.right='';el.style.bottom='';changed=true;}
if(changed&&save){localStorage.setItem(key,JSON.stringify({left:el.style.left,top:el.style.top}));}}
...
function injectUI(){
  ...
  // if saved pos was off-screen, snap back
  ...
  function refreshLeaderHint(h){ if(!h)return; h.textContent=(isLeader() ? 'Denne fane er LEADER for push.' : 'Ikke leader – en anden fane sender push.'); }
  ...
  function buildMenu(){
    ...
    up.addEventListener('click',async()=>{try{const token=(pat.value||'').trim();if(!token){showToast('Indsæt GitHub PAT først.');return;}if(!file.files||!file.files[0]){showToast('Vælg en CSV-fil først.');return;}const text=await file.files[0].text();const base64=b64Utf8(text);pbh.textContent='Uploader CSV…';const {sha}=await ghGetSha(PB_OWNER,PB_REPO,PB_CSV,PB_BRANCH);await ghPutFile(PB_OWNER,PB_REPO,PB_CSV,base64,'sync: upload CSV via TM',sha,PB_BRANCH);pbh.textContent='CSV uploadet. RAW opdateres om få sek.';showToast('CSV uploadet.');}catch(e){tpLog('warn','[CSV-UPLOAD]',e);pbh.textContent='Fejl ved CSV upload.';showToast('Fejl – se konsol.');}});
    csvUp.addEventListener('click',async()=>{try{pbh.textContent='Henter Excel → vælger bedste ark → CSV → upload …';const t0=Date.now();await fetchExcelAsCSVAndUpload();pbh.textContent=`Færdig på ${Date.now()-t0} ms.`;}catch(e){tpLog('warn','[EXCEL→CSV]',e);pbh.textContent='Fejl ved Excel→CSV upload.';showToast('Fejl – se konsol.');}});
    patTest.addEventListener('click',async()=>{try{const token=(pat.value||'').trim();if(!token){showToast('Indsæt PAT først.');return;}const headers={'Accept':'application/vnd.github+json','Authorization':'Bearer '+token,'X-GitHub-Api-Version':'2022-11-28'};const rl=await new Promise((res,rej)=>GM_xmlhttpRequest({method:'GET',url:'https://api.github.com/rate_limit',headers,onload:r=>res(r),onerror:rej}));const repo=await new Promise((res,rej)=>GM_xmlhttpRequest({method:'GET',url:`https://api.github.com/repos/${PB_OWNER}/${PB_REPO}`,headers,onload:r=>res(r),onerror:rej}));if(rl.status===200&&repo.status===200){pbh.textContent='PAT OK – adgang til repo og rate limit aktiv.';showToast('PAT OK');}else{pbh.textContent=`PAT problem: rate=${rl.status} repo=${repo.status}`;showToast('PAT problem – se konsol.');}}catch(e){tpLog('warn','[PAT-TEST]',e);showToast('PAT test fejlede.');}});

    // live leader status while open
    const tick=()=>refreshLeaderHint(hint);
    leaderTimer=setInterval(tick,1000); tick();
    return menu;
  }

  function toggleMenu(){
    if(event && event.shiftKey){ // shift + click on gear = reset panel pos instantly
      localStorage.removeItem('tpPosPanel'); d.style.left=''; d.style.top=''; d.style.right='8px'; d.style.bottom='8px'; showToast('Panel reset.');
      return;
    }
    const mnu=buildMenu(); const r=gear.getBoundingClientRect();
    mnu.style.right=(window.innerWidth-r.right)+'px'; mnu.style.bottom=(window.innerHeight-r.top+6)+'px';
    mnu.style.display=(mnu.style.display==='block'?'none':'block');
    if(mnu.style.display==='none' && leaderTimer){clearInterval(leaderTimer); leaderTimer=null;}
    if(mnu.style.display==='block' && !leaderTimer){leaderTimer=setInterval(()=>{const h=mnu.querySelector('#tpLeaderHint');h&& (h.textContent=isLeader()?'Denne fane er LEADER for push.':'Ikke leader – en anden fane sender push.');},1000);}
  }
...
function tpTest(){const k=getUserKey();if(!k){showToast('Indsæt USER-token i ⚙️ først.');return;}const ts=new Date().toLocaleTimeString();sendPushover('🧪 [TEST] Besked-kanal OK — '+ts).catch(()=>{});setTimeout(()=>sendPushover('🧪 [TEST] Interesse-kanal OK — '+ts).catch(()=>{}),800);showToast('Sendte Pushover-test (Besked + Interesse).');}
...
pollMessages().catch(()=>{}); pollInterest().catch(()=>{}); schedule();
injectUI();
