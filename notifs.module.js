...
/**
 * TPNotifs — Besked/Interesse-notifikationer + Pushover + toasts (modul)
 * Brug: TPNotifs.install({...})
 *
 * Public API:
 *   TPNotifs.install(opts)  // sæt config og start pollerne
 *   TPNotifs.start()        // start pollerne
 *   TPNotifs.stop()         // stop pollerne
 *   TPNotifs.testPushover() // send test-beskeder (kræver tpUserKey i GM)
 *   TPNotifs._cfg()         // læs aktiv konfiguration (readonly copy)
 */
(function () {
  'use strict';

  const MOD_VER = 'v2025-08-28-02';
  try { console.info('[TP] notifs.module', MOD_VER, 'loaded at', new Date().toISOString()); } catch (_) {}

  /* ---------- Config (defaults) ---------- */
  const DEF = Object.freeze({
    pushoverToken: '', // kræver at brugeren har sat GM 'tpUserKey'
    pollMs: 15000,
    suppressMs: 45000,
    lockMsExtra: 5000,
  ...
  const DBG = (localStorage.getItem('tpDebug') === '1');

  function dbg(...a){ if (DBG) { try { console.log('[TPNotifs]', ...a); } catch(_){} } }

  function loadJson(key, fb) {
    try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(fb)); }
    catch (_) { return JSON.parse(JSON.stringify(fb)); }
  }
  ...
  /* ---------- Cross-tab broadcast for DOM-toasts ---------- */
  ...
  window.addEventListener('storage', e => {
    ...
    try {
      ...
      localStorage.setItem(seenKey, '1'); // dedupe
      showDOMToast(ev.msg);
    } catch (_) {}
  });
  ...
  const gIntNamesCache = new Map(); // vagtAvailId -> { ts, names[] }

  function markParsedNow() { lastIntParseTS = Date.now(); }
  function mustForceParse() { return (Date.now() - lastIntParseTS) > (CFG.pollMs * 2); }
  ...
  function parseInterestHTML(html) {
    ...
    const c = boxes.reduce((s, el) => s + (parseInt((el.textContent || '').replace(/\D+/g, ''), 10) || 0), 0);
    return c;
  }
  ...
  function parseInterestPerMap(html) {
    ...
    for (const el of boxes) {
      ...
      if (!m) continue;
      const vagtId = m[1];
      const v = parseInt((el.textContent || '').replace(/\D+/g, ''), 10) || 0;
      map[vagtId] = v;
    }
    return map;
  }

  // CSV helpers (vikarId -> name)
  ...
  function buildVikarIdMap(csv) {
    ...
    if (idxId < 0) return res;
    for (let r = 1; r < rows.length; r++) {
      const row = rows[r];
      const id = (row[idxId] || '').trim();
      const nm = idxName >= 0 ? (row[idxName] || '').trim() : '';
      if (id) res.set(String(id), nm || '');
    }
    return res;
  }

  let _vikarNameById = null; // Map
  async function ensureVikarNameMap() {
    if (_vikarNameById && _vikarNameById.size) return _vikarNameById;
    let csv = '';
    try {
      csv = await gmGET(CFG.rawPhonebookUrl + '?t=' + Date.now());
      if (csv) GM_setValue(CFG.cacheKeyCSV, csv);
    } catch (_) { /* fallback til cache */ }
  ...
  function parseInterestPopupNames(html, lookupByVikarId) {
    const doc = new DOMParser().parseFromString(html, 'text/html');
    // Både "interresse" (ældre) og "interesse" (korrekt)
    const rows = Array.from(
      doc.querySelectorAll('.vikar_interresse_list_container, .vikar_interesse_list_container')
    );
    const out = [];
    for (const row of rows) {
      ...
      if (!vikarId) {
        const a = (
          row.querySelector('.vikar_interresse_list_remove_container a, .vikar_interesse_list_remove_container a') || null
        );
        const on = a && a.getAttribute && a.getAttribute('onclick') || '';
        m = on.match(/removeVagtInteresse\((\d+)\s*,/);
        if (m) vikarId = m[1];
      }
      // navn-container i begge stavemåder
      let name = (
        row.querySelector('.vikar_interresse_list_navn_container, .vikar_interesse_list_navn_container') || {}
      ).textContent?.trim() || '';

      // Forkortet? enten “…”, eller “Anne J.” → prøv CSV for fuldt navn
      const isAbbrev = /\.\.\.$/.test(name) || /\s[A-ZÆØÅ]\.$/.test(name);
      if (isAbbrev && vikarId && lookupByVikarId) {
            ...
            .then(async html => {
              ...
              if (CFG.enableInterestNameHints && rising.length) {
                ...
                for (const vagtId of toFetch) {
                  ...
                  if (cached && (ts - cached.ts) < INT_NAMES_CACHE_TTL_MS && cached.names?.length) {
                    collected.push(...cached.names);
                    continue;
                  }
                  try {
  ...
  function install(opts = {}) {
    ...
    // ryd legacy global lock-key fra gammel version
    try { localStorage.removeItem('tpPushLock'); } catch (_) {}

    // prime CSV i baggrunden (til navn-hints)
  ...
  function testPushover() {
    const user = getUserKey();
    if (!user) { showToastOnce('test', 'Indsæt din Pushover USER-token i ⚙️ først.'); return; }
