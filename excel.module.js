...
(function(){
  ...
  function parsePhonebookCSV(text){
    const vikarsById=new Map(); const map=new Map(); const rows=parseCSV(text); if(!rows.length) return { map, header:[], vikarsById };
  ...
  async function fetchExcelAsCSVText(){
    ...
    for(const t of tries){
      try{
        ...
        const wb=XLSX.read(ab,{type:'array'}); if(!wb.SheetNames||wb.SheetNames.length===0) continue;
  ...
  async function fetchExcelAsCSVAndUpload(){
    ...
    const lines=(text||'').trim().split(/\r?\n/).filter(Boolean); if(lines.length<2) return { ok:false, reason:'only-header' };
  ...
  function attachToMenu(menu){
    ...
    if (pat){ pat.value = getPAT(); pat.addEventListener('input', ()=> setPAT(pat.value||'')); }
    ...
    if (up){
      up.addEventListener('click', async ()=>{
        ...
        } catch(e){ console.warn('[TPExcel][CSV-UPLOAD]', e); pbh.textContent='Fejl ved CSV upload (se konsol).'; }
      });
    }
    ...
    if (csvUp){
      csvUp.addEventListener('click', async ()=>{
        ...
        } catch(e){ console.warn('[TPExcel][EXCEL→CSV-UPLOAD]', e); pbh.textContent='Fejl ved Excel→CSV upload (se konsol).'; }
      });
    }
    ...
    if (tBtn){
      tBtn.addEventListener('click', async ()=>{
        ...
        } catch(e){ console.warn('[TPExcel][LOOKUP]', e); pbh.textContent='Fejl ved opslag.'; }
      });
    }
  }
