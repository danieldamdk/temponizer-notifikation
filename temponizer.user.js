...
// @version      7.10.4
// @description  Push (leader, suppression), OS/DOM toast, “Intet Svar”-auto-gem (uden popup), telefonbog m. inbound caller-pop (nyt faneblad), Excel→CSV→Upload til GitHub, RAW CSV lookup. Statusbanner, navne i interesse-notifikationer og “Søg efter opdatering” i ⚙️. Overlevering fjernet. UI smallere, én SMS-toggle.
...
function injectUI() {
  ...
  d.innerHTML =
    ...
    '</div>' +
    // Pushover toggles + badges (labels uden "(Pushover)")
    ...
    '</div>' +
    // SMS (smal: én toggle-knap under status)
  ...
  if (!document.getElementById('tpGear')) {
    ...
    function toggleMenu() {
      ...
      if (mnu.style.display === 'block') {
        // placer under/over panel, men aldrig udenfor viewport
        const mw = mnu.offsetWidth, mh = mnu.offsetHeight;
        let left = Math.max(8, Math.min(window.innerWidth - mw - 8, pr.left));
        let top  = pr.bottom + 8;
        if (top + mh > window.innerHeight - 8) top = Math.max(8, pr.top - mh - 8);
        Object.assign(mnu.style, { left:left+'px', top:top+'px' });
      }
    }
...
/*──────── 12b) SMS (iframe-metode så det virker på ALLE sider) ────────*/
...
const sms = {
  ...
  async refresh(cb) {
    try {
      ...
      this._last = { state: 'unknown' }; cb && cb(this._last);
    }
  },
