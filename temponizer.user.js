...
// @version      7.9.3
// @description  Push (leader, suppression), toast (Smart: DOM når synlig, OS når skjult • max 1 OS), “Intet Svar”-auto-gem, telefonbog m. inbound caller-pop (kun kø *1500, nyt faneblad, nul flash), Excel→CSV→Upload til GitHub, RAW CSV lookup. Statusbanner, “Søg efter opdatering”, drag af UI + CSV drag&drop.
...
function showToast(msg) {
  ...
  // 2) SMART → DOM hvis fanen er synlig
...
async function fetchExcelAsCSVText() {
  const tries = [
    ...
    { fn: tryExcelGET,  params: 'id=true&name=true&phone=true&cellphone=true' }, // rettet
...
function injectUI() {
  ...
  if (!document.getElementById('tpGear')) {
    ...
    function buildMenu() {
      ...
      // Manuelt update-tjek (supplerer TM auto-update)
