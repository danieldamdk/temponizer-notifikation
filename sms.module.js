...
(function(){
  ...
  function parseSmsStatusFromDoc(doc){
    ...
    const hasDeact = !!(doc.querySelector('#sms_notifikation_aktiv a[onclick*="deactivate_cell_sms_notifikationer"]')
  ...
  function parseSmsStatusFromHTML(html){
    return parseSmsStatusFromDoc(new DOMParser().parseFromString(html,'text/html'));
  }
  ...
  async function fetchSmsStatusHTML(){
    return gmGET(CFG.settingsUrl + '&t=' + Date.now());
  }
  ...
  function invokeIframeAction(ifr, wantOn){
    const w = ifr.contentWindow, d = ifr.contentDocument;
    try {
      if (wantOn && typeof w.activate_cell_sms_notifikationer === 'function'){ w.activate_cell_sms_notifikationer(); return true; }
      if (!wantOn && typeof w.deactivate_cell_sms_notifikationer === 'function'){ w.deactivate_cell_sms_notifikationer(); return true; }
    } catch(_){}
    try {
      const link = wantOn
  ...
  async function toggleSmsInIframe(wantOn, timeoutMs=15000, pollMs=500){
    ...
    const maybeReloaded = new Promise(res=>{
      let done=false;
      ifr.addEventListener('load', ()=>{ if(!done){ done=true; res(); } }, { once:true });
      setTimeout(()=>{ if(!done) res(); }, 1200);
    });
    await maybeReloaded;
    const t0 = Date.now();
  ...
  function bindUI(container){
    ...
    function setBusy(on, text){
      if (btn){ btn.disabled = on; btn.style.opacity = on ? .6 : 1; }
      if (on && text && lbl) lbl.textContent = text;
    }
    function paint(st){
      if (!lbl || !btn) return;
      switch (st.state){
        case 'active':
          btn.textContent='Deaktiver'; lbl.textContent='SMS: Aktiv' + (st.phone?(' — '+st.phone):''); lbl.style.color='#0a7a35'; break;
        case 'inactive':
          btn.textContent='Aktivér'; lbl.textContent='SMS: Ikke aktiv' + (st.phone?(' — '+st.phone):''); lbl.style.color='#a33'; break;
        default:
          btn.textContent='Aktivér'; lbl.textContent='SMS: Ukendt'; lbl.style.color='#666';
      }
    }
  ...
  function ensureSection(){
    ...
    if (!root){
      ...
      root.style.cssText = 'position:fixed;right:12px;bottom:12px;background:#fff;border:1px solid #ccc;border-radius:8px;padding:8px 10px;box-shadow:0 8px 24px rgba(0,0,0,.15);font:12px system-ui,sans-serif;z-index:2147483645';
  ...
  const TPSms = {
    install(opts={}){
      CFG = { ...DEF, ...(opts||{}) };
      const sec = ensureSection(); bindUI(sec);
  ...
  try { window.TPSms = Object.freeze(TPSms); } catch(_) { window.TPSms = TPSms; }
})();
