...
// @version      7.11.5
// @description  Pushover + OS/DOM toast (no dupes, på tværs af faner). “Intet Svar”-auto. Pushover ved Besked/Interesse + én SMS-aktiver/deaktiver-knap. Kompakt UI nederst-højre + ⚙️ til Pushover token. (Caller-pop/telefonbog fjernet)
...
function injectUI() {
  ...
  if (!document.getElementById('tpGear')) {
    ...
    function buildMenu() {
      ...
      Object.assign(menu.style, {
        ...
        padding:'12px', width:'360px', maxHeight:'70vh', overflow:'auto', display:'none'
      });
      ...
      menu.innerHTML =
        ...
        '<div style="margin-bottom:10px">' +
          ...
          '<div style="margin-top:6px;display:flex;gap:6px;align-items:center;flex-wrap:wrap">' +
            ...
            '<button id="tpTestPushoverBtn" style="padding:6px 8px;border:1px solid #ccc;border-radius:6px;background:#fff;cursor:pointer">🧪 Test</button>' +
            '<a href="https://pushover.net/" target="_blank" rel="noopener" style="margin-left:auto;color:#06c;text-decoration:none">Pushover</a>' +
          '</div>' +
          '<div style="margin-top:6px;font-size:11px;color:#666">Tip: Slå Besked/Interesse til i panelet for at sende notifikationer.</div>' +
        '</div>' +
    ...
    function positionMenu(menu) {
      ...
      const mw = Math.min(menu.offsetWidth || 360, window.innerWidth - 16);
  ...
  // toggles
  const cbMsg = d.querySelector('#tpEnableMsg');
  const cbInt = d.querySelector('#tpEnableInt');

  cbMsg.checked = localStorage.getItem('tpPushEnableMsg') === 'true';
  cbInt.checked = localStorage.getItem('tpPushEnableInt') === 'true';

  cbMsg.addEventListener('change', () => {
    localStorage.setItem('tpPushEnableMsg', cbMsg.checked ? 'true' : 'false');
    showToast(cbMsg.checked ? 'Besked-notifikationer slået til.' : 'Besked-notifikationer slået fra.');
  });
  cbInt.addEventListener('change', () => {
    localStorage.setItem('tpPushEnableInt', cbInt.checked ? 'true' : 'false');
    showToast(cbInt.checked ? 'Interesse-notifikationer slået til.' : 'Interesse-notifikationer slået fra.');
  });

  // drag panel + remember position
  const header = d.querySelector('#tpHeader');
  const hint = d.querySelector('#tpDragHint');
  ...
  if (saved && typeof saved.left === 'number' && typeof saved.top === 'number') {
    d.style.left = saved.left + 'px';
    d.style.top  = saved.top + 'px';
  } else {
    d.style.right = '16px';
    d.style.bottom = '54px';
  }
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    d.style.left = (drag.l + dx) + 'px';
    d.style.top  = (drag.t + dy) + 'px';
  ...
  document.addEventListener('mouseup', () => {
    ...
    const r = d.getBoundingClientRect();
    saveJson(POS_KEY, { left: Math.round(r.left), top: Math.round(r.top) });
  });

  // init badges
  try {
    const stMsg = loadJson(ST_MSG_KEY, {count:0,lastPush:0,lastSent:0,pending:0});
    const stInt = loadJson(ST_INT_KEY, {count:0,lastPush:0,lastSent:0,pending:0});
    setBadge(document.getElementById('tpMsgCountBadge'), stMsg.count || 0);
    setBadge(document.getElementById('tpIntCountBadge'), stInt.count || 0);
  } catch(_) {}
}

/*──────── 13) SMS (én knap: Aktivér/Deaktivér via iframe) ────────*/
...
(async function initSMS(){
  try {
    ...
    console.warn('[TP][SMS][INIT]', e);
  }
})();

/*──────── 15) “Intet Svar” auto (beholdt) ────────*/
(function(){
  let auto = false;
