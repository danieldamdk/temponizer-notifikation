...
(function(){
      ...
      GM_xmlhttpRequest({
        ...
        onload:r=>{
          if (r.status===200){
            ...
            catch{ resolve({ sha:null, exists:true }); }
  ...
  async function ghPutFile(path, base64Content, message, sha, branch){
    ...
    return new Promise((resolve, reject)=>{
      GM_xmlhttpRequest({
        ...
        onload:r=>{ if (r.status===200||r.status===201) resolve(r.responseText); else reject(new Error('GitHub PUT: HTTP '+r.status+' :: '+String(r.responseText||'').slice(0,260))); },
        onerror:reject
      });
    });
  }
  ...
  function parseCSV(text){
    ...
    while(i<text.length){
      ...
      if(c==='\r'){ i++; continue;}
      if(c==='\n'){ row.push(f.trim()); rows.push(row); row=[]; f=''; i++; continue;}
      if(c===delim){ row.push(f.trim()); f=''; i++; continue;}
      f+=c; i++;
    }
  ...
  function parsePhonebookCSV(text){
    const vikarsById=new Map(); const map=new Map();
    const rows=parseCSV(text); if(!rows.length) return { map, header:[], vikarsById };
    const hdr=rows[0].map(h=>h.toLowerCase());
    const idxId=hdr.findIndex(h=>/(vikar.*nr|vikar[_ ]?id|^id$)/.test(h));
    const idxName=hdr.findIndex(h=>/(navn|name)/.test(h));
    const phoneCols=hdr.map((h,idx)=>({h,idx})).filter(x=>/(telefon|mobil|cellphone|mobile|phone|tlf)/.test(x.h));
    if(idxId<0||phoneCols.length===0) return { map, header:hdr, vikarsById };
    ...
    return { map, header:hdr, vikarsById };
  }

  // Excel flow (samme som din nuværende – kun små tweaks)
  ...
  function pickBestSheetCSV(wb){
    ...
    for(const nm of wb.SheetNames){
      ...
      let csv=XLSX.utils.sheet_to_csv(sh,{FS:',',RS:'\n'});
  ...
  async function warmUpVikarListSession(){
    ...
    const body='page=vikarlist_get&ajax=true&showheader=true&printlist=true'
  ...
  async function fetchExcelAsCSVText(){
    try{ await warmUpVikarListSession(); } catch{}
  ...
  async function fetchExcelAsCSVAndUpload(){
    const pat=getPAT(); if(!pat) throw new Error('Missing GitHub PAT');
    const text=await fetchExcelAsCSVText(); if(!text) return { ok:false, reason:'no-data' };
    const lines=(text||'').trim().split(/\r?\n/).filter(Boolean);
    if(lines.length<2) return { ok:false, reason:'only-header' };
    const base64=b64encodeUtf8(text);
    const { sha } = await ghGetSha(CFG.csvPath, CFG.branch);
    await ghPutFile(CFG.csvPath, base64, 'sync: Excel→CSV via TM (auto)', sha, CFG.branch);
    GM_setValue(CFG.cacheKeyCSV, text);
    return { ok:true, lines: lines.length-1 };
  }

  function attachToMenu(menu){
    if(!menu) return;
    ...
    if (pat){
      pat.value = getPAT();
      pat.addEventListener('input', ()=> setPAT(pat.value||''));
    }
    if (up){
      up.addEventListener('click', async ()=>{
        try{
          setPAT((pat?.value||'').trim());
          if(!getPAT()){ pbh.textContent='Mangler GitHub PAT i ⚙️ → Telefonbog.'; return; }
          if(!file?.files||!file.files[0]){ pbh.textContent='Vælg en CSV-fil først.'; return; }
          const text = await file.files[0].text();
          const base64=b64encodeUtf8(text);
          pbh.textContent='Uploader CSV…';
          const { sha } = await ghGetSha(CFG.csvPath, CFG.branch);
          await ghPutFile(CFG.csvPath, base64, 'sync: upload CSV via TM', sha, CFG.branch);
          GM_setValue(CFG.cacheKeyCSV, text);
          pbh.textContent='CSV uploadet. RAW opdateres om få sek.';
        } catch(e){
          console.warn('[TPExcel][CSV-UPLOAD]', e);
    ...
    if (csvUp){
      csvUp.addEventListener('click', async ()=>{
        try{
          ...
          const t0=Date.now();
          const res=await fetchExcelAsCSVAndUpload();
          const ms=Date.now()-t0;
          pbh.textContent = res?.ok ? `Færdig på ${ms} ms. Rækker: ${res.lines}` : `Ingen data (${res?.reason||'ukendt'}).`;
        } catch(e){
          console.warn('[TPExcel][EXCEL→CSV-UPLOAD]', e);
    ...
    if (tBtn){
      tBtn.addEventListener('click', async ()=>{
        try{
          ...
          let csv='';
          try{
            csv=await gmGET(`https://raw.githubusercontent.com/${CFG.owner}/${CFG.repo}/${CFG.branch}/${CFG.csvPath}?t=${Date.now()}`);
            if(csv) GM_setValue(CFG.cacheKeyCSV, csv);
          }catch{}
          if(!csv) csv = GM_getValue(CFG.cacheKeyCSV)||'';
          const { map } = parsePhonebookCSV(csv);
          const rec = map.get(p8);
          pbh.textContent = rec ? `Match: ${p8} → ${rec.name || '(uden navn)'} (vikar_id=${rec.id})` : `Ingen match for ${p8}.`;
          if (rec){
            const url = `/index.php?page=showvikaroplysninger&vikar_id=${encodeURIComponent(rec.id)}#stamoplysninger`;
            window.open(url,'_blank','noopener');
          }
        } catch(e){
          console.warn('[TPExcel][LOOKUP]', e);
  ...
  const TPExcel = {
    install(opts={}){ CFG = { ...DEF, ...(opts||{}) }; log('installed with', CFG); },
    attachToMenu,
  ...
  try { window.TPExcel = Object.freeze(TPExcel); } catch(_) { window.TPExcel = TPExcel; }
})();
