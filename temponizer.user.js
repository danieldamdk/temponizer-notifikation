...
// @version      7.4
// @description  CSV sync & lookup with status + fallback cache; Excel→CSV uses fetchIds; update-check; inbound caller-pop only when number ends with *1500; push (leader), hover “Intet Svar”, draggable panel.
...
if (window.__TP74_RUNNING__) { console.info('[TP] already running'); } else { window.__TP74_RUNNING__ = true;
...
/* IDs */
function collectVisibleVikarIds(doc=document){
  const ids=new Set();
...
async function collectIdsViaAjax(){
  const form=document.querySelector('form#vikarsearchform'); if(!form) return [];
...
/* Excel→CSV (uses fetchIds where possible) */
async function fetchExcelAsCSVText(){
  await ensureXLSX();

  let ids=collectVisibleVikarIds(document);
  if(!ids.length){ try{ ids=await collectIdsViaAjax(); }catch(_){ /* noop */ } }
  ...
  // Fallbacks (kan give 0 rækker for nogle setups)
  const tries=[
...
async function fetchExcelAsCSVAndUpload(statusEl){
  statusEl && (statusEl.textContent='Henter Excel → CSV …');
  const text=await fetchExcelAsCSVText();
  if(!text){ statusEl && (statusEl.textContent='Ingen rækker – CSV beholdes.'); showToastOnce('csv','Ingen rækker fra Temponizer – beholdt eksisterende CSV.'); return; }
  const lines=text.trim().split(/\r?\n/).filter(Boolean);
  if(lines.length<2){ statusEl && (statusEl.textContent='Kun header – CSV beholdes.'); showToastOnce('csv','Kun header – beholdt eksisterende CSV.'); return; }
  ...
  // Opdater lokal cache for hurtige opslag
  GM_setValue(CACHE_KEY_CSV, text);

  statusEl && (statusEl.textContent='CSV uploadet. (RAW kan være 10–30s om at opdatere)');
  showToastOnce('csvok','CSV uploadet. (RAW opdateres typisk på 10–30s)');
}

/* Caller-pop (ONLY when number ends with *1500) */
let _lastPopAt=0;
async function callerPopIfNeeded(){
  try{
    ...
    // Strict inbound: ONLY accept *1500 suffix (ignore tp_dir)
    const inbound=/\*1500$/.test(String(raw));
    if(!inbound) {
      // Clean up URL quickly so tab looks “benign”
      try { history.replaceState(null,'',location.origin+'/index.php?page=front'); } catch(_){}
      return;
    }
    ...
    // Try RAW; fallback to local cache
    let csvText='';
    try { csvText = await gmGET(RAW_PHONEBOOK+'?t='+(Date.now())); }
    catch(_) { /* ignore */ }
...
async function checkForUpdate(statusEl){
  try{
    ...
    if(remote === TP_VER){
      statusEl && (statusEl.textContent='Du kører nyeste version ('+TP_VER+').');
      showToast('All good – du kører '+TP_VER);
    }else{
      statusEl && (statusEl.textContent='Ny version tilgængelig: '+remote+' (du har '+TP_VER+')');
      showToast('Ny version: '+remote+' (du har '+TP_VER+'). Tampermonkey vil normalt auto-opdatere.');
    }
  }catch(e){
    statusEl && (statusEl.textContent='Version-tjek fejlede.');
    console.warn('[TP][UPDATE]', e);
  }
}
...
function injectUI(){
  ...
  function buildMenu(){
    ...
    csvUp.addEventListener('click',async()=>{try{
      ...
      pbh.textContent='Henter Excel → (fetchIds) → CSV → upload …';
    ...
    // CSV lookup with visible status + local fallback cache
